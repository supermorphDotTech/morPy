...
class cl_progress_gui:
    ...
    def _init(self, mpy_trace: dict, app_dict: dict, frame_title: str = None, frame_width: int = 800,
        ...
        try:
            ...
            # Default texts
            self.frame_title = (app_dict["loc"]["mpy"]["cl_progress_gui_prog"]
                                if not frame_title else frame_title)
            self.frame_width = frame_width
            self.frame_height = frame_height
            self.headline_total_nocol = (f'{app_dict["loc"]["mpy"]["cl_progress_gui_overall"]}'
                             if not headline_total else f'{headline_total}')
            self.headline_stage_nocol = (f'{app_dict["loc"]["mpy"]["cl_progress_gui_curr"]}'
            ...
            self.font = font
            self.button_text_abort = f'{app_dict["loc"]["mpy"]["cl_progress_gui_abort"]}'
            self.button_text_close = f'{app_dict["loc"]["mpy"]["cl_progress_gui_close"]}'
            ...
            if self.stages > 1:
                ...
                self.overall_progress_tracker = mpy_common.cl_progress(
                    mpy_trace, app_dict, description=self.headline_total_nocol, total=self.stages, ticks=.01
                )
            ...
            if self.max_per_stage > 0:
                ...
                self.stage_progress_tracker = mpy_common.cl_progress(
                    mpy_trace, app_dict, description=self.headline_stage_nocol, total=self.max_per_stage, ticks=.01
                )
    ...
    def _create_widgets(self, mpy_trace: dict, app_dict: dict):
        ...
        try:
            ...
            self.close_button = tk.Button(
                ...
                command=lambda: self._on_close(mpy_trace, app_dict)
            )
    ...
    @metrics
    def _redirect_console(self, mpy_trace: dict, app_dict: dict):
        r"""
    ...
    @metrics
    def _stop_console_redirection(self, mpy_trace: dict, app_dict: dict):
        r"""
    ...
    @metrics
    def _main_loop(self, mpy_trace, app_dict):
        r"""
        ...
        try:
            ...
            # Schedule again
            self.root.after(self.main_loop_interval, lambda: self._main_loop(mpy_trace, app_dict))
    ...
    @metrics
    def update_progress(self, mpy_trace: dict, app_dict: dict, current: float = None, max_per_stage: int = None):
        r"""
        Update stage progress & overall progress. If overall hits 100% and auto_close=True, close window. Else,
        switch button text to "Close" and stop console redirection.
        ...
        :example:
            ...
            progress.update_progress(mpy_trace, app_dict, current=curr_cnt, description_stage=msg)
            ...
            # Setup stage 2
            progress.update_text(mpy_trace, app_dict,
                headline_stage="Starting stage 2",
                description_stage="Now copying data...",
            )
            progress.update_progress(mpy_trace, app_dict, current=0, max_per_stage=15)
        ...
        try:
            ...
            if self.done:
                ...
                # 1) stage progress
                if self.stage_progress_on:
                    stage_info = self.stage_progress_tracker.update(mpy_trace, app_dict, current=current)
    ...
    @metrics
    def update_text(self, mpy_trace: dict, app_dict: dict, headline_total: str = None, headline_stage: str = None,
                    description_stage: str = None):
        r"""
        Update the headline texts or description at runtime.
        ...
        :example:
            gui.update_text(mpy_trace, app_dict,
    ...
    def get_console_output(self, mpy_trace: dict, app_dict: dict):
        ...
        :example:
            self.get_console_output(mpy_trace, app_dict)
        """
