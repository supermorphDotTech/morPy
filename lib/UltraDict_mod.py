...
class UltraDict(collections.UserDict, dict):
    ...
    def __setstate__(self, state):
        ...
        # Reinitialize shared memory views
        self.init_remotes()

    def __getattr__(self, attr):
        if attr == 'full_dump_memory_name_remote':
            # Try to attach if missing
            mem_name = self.get_full_dump_memory().name  # or some appropriate call
            self.__dict__['full_dump_memory_name_remote'] = mem_name.encode('utf-8').ljust(255)
            return self.__dict__['full_dump_memory_name_remote']
    ...
    def __getitem__(self, key):
        ...
        if isinstance(value, dict) and not isinstance(value, NotifyingDict):
            value = NotifyingDict(value, parent=self, parent_key=key)
            self.data[key] = value
        elif isinstance(value, list) and not isinstance(value, NotifyingList):
            value = NotifyingList(value, parent=self, parent_key=key)
            self.data[key] = value
        elif isinstance(value, set) and not isinstance(value, NotifyingSet):
            value = NotifyingSet(value, parent=self, parent_key=key)
            self.data[key] = value
        return value
