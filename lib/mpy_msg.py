...
def log(mpy_trace, prj_dict, log_message, level):
    ...
    if prj_dict["conf"]["mpy_msg_print"] or prj_dict["conf"]["mpy_log_enable"]:
    ...
    if prj_dict["conf"]["mpy_log_enable"] and \
        log_dict["log_enable"] and \
        prj_dict["conf"]["mpy_log_txt_enable"]:
...
def log_eval(mpy_trace, prj_dict, level, level_dict):
    ...
    if prj_dict["conf"]["mpy_log_enable"]:
        
        # Evaluate the log level, if it is excluded from logging.
        for lvl_nolog in prj_dict["conf"]["mpy_log_lvl_nolog"]:
    ...
    if prj_dict["conf"]["mpy_msg_print"]:
        ...
        for lvl_noprint in prj_dict["conf"]["mpy_log_lvl_noprint"]:
    ...
    for lvl_intpt in prj_dict["conf"]["mpy_log_lvl_interrupts"]:
...
def log_interrupt(mpy_trace, prj_dict):
    ...
    msg_text = prj_dict["loc"]["mpy"]["mpy_msg_print_intrpt"]
...
def log_msg_builder(prj_dict, log_dict):
    ...
    msg = (f'{log_dict["level"]} - {log_dict["datetimestamp"]}\n\t'
          f'{prj_dict["loc"]["mpy"]["log_msg_builder_trace"]}: {log_dict["tracing"]}\n\n\t'
          f'{prj_dict["loc"]["mpy"]["log_msg_builder_process_id"]}: {log_dict["process_id"]}\n\t'
          f'{prj_dict["loc"]["mpy"]["log_msg_builder_thread_id"]}: {log_dict["thread_id"]}\n\n'
          f'{msg_indented}\n')
...
def mpy_msg_print(mpy_trace, prj_dict, log_dict):
    ...
    for lvl_pnt in prj_dict["conf"]["mpy_log_lvl_noprint"]:
...
def log_db(mpy_trace, prj_dict, log_dict):
    ...
    db_path = prj_dict["conf"]["log_db_path"]
    table_name = f'log_{prj_dict["run"]["init_loggingstamp"]}'
...
def log_db_connect(mpy_trace, prj_dict, db_path):
    ...
    # Error detection
        ...
        log_message = (f'{prj_dict["loc"]["mpy"]["err_line"]}: {sys.exc_info()[-1].tb_lineno}\n'
                      ...
                      f'{prj_dict["loc"]["mpy"]["log_db_connect_excpt"]}\n'
                      f'db_path: {db_path}')
...
def log_db_disconnect(mpy_trace, prj_dict, db_path):
    ...
    # Error detection
        ...
        log_message = (f'{prj_dict["loc"]["mpy"]["err_line"]}: {sys.exc_info()[-1].tb_lineno}\n'
                      f'{prj_dict["loc"]["mpy"]["err_excp"]}: {e}\n'
                      f'{prj_dict["loc"]["mpy"]["log_db_disconnect_excpt"]}\n'
                      f'db_path: {db_path}')
        log(mpy_trace, prj_dict, log_message, 'critical')
...
def log_db_table_create(mpy_trace, prj_dict, db_path, table_name):
    ...
    # Error detection
        ...
        log_message = (f'{prj_dict["loc"]["mpy"]["err_line"]}: {sys.exc_info()[-1].tb_lineno}\n'
                      f'{prj_dict["loc"]["mpy"]["err_excp"]}: {e}\n'
                      f'{prj_dict["loc"]["mpy"]["log_db_table_create_excpt"]}\n'
                      f'{prj_dict["loc"]["mpy"]["log_db_table_create_stmt"]}: {exec_statement}')
        log(mpy_trace, prj_dict, log_message, 'critical')
        ...
        if check:
            ...
            for col in columns:
                ...
                # Error detection
                    ...
                    log_message = (f'{prj_dict["loc"]["mpy"]["err_line"]}: {sys.exc_info()[-1].tb_lineno}\n'
                                  f'{prj_dict["loc"]["mpy"]["err_excp"]}: {e}\n'
                                  f'{prj_dict["loc"]["mpy"]["log_db_table_add_column_excpt"]}\n'
                                  f'{prj_dict["loc"]["mpy"]["log_db_table_add_column_stmt"]}: {exec_statement}')
                    log(mpy_trace, prj_dict, log_message, 'critical')
    ...
    try:
        ...
        if check:
            ...
            # Error detection
                ...
                log_message = (f'{prj_dict["loc"]["mpy"]["err_line"]}: {sys.exc_info()[-1].tb_lineno}\n'
                              f'{prj_dict["loc"]["mpy"]["err_excp"]}: {e}\n'
                              f'{prj_dict["loc"]["mpy"]["log_db_row_insert_excpt"]}\n'
                              f'{prj_dict["loc"]["mpy"]["log_db_row_insert_stmt"]}: {exec_statement}')
                log(mpy_trace, prj_dict, log_message, 'critical')
