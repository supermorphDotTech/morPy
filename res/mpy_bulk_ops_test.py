...
class cl_ut_find_replace_saveas(unittest.TestCase):
    ...
    def ut_find_replace_saveas(self, mock_textfile_write, mock_regex_replace, mock_fso_delete_file, mock_pathtool, mock_log, mock_tracing):
        ...
        mock_log.assert_any_call(self.mpy_trace, self.prj_dict, self.prj_dict["find_replace_saveas_start"], 'debug')
        mock_regex_replace.assert_any_call(self.mpy_trace, self.prj_dict, "Let's replace1 and replace2!", "replace1", "with1")
    ...
    def ut_find_replace_saveas_file_exists_no_overwrite(self, mock_pathtool, mock_log):
        ...
        mock_log.assert_any_call(self.mpy_trace, self.prj_dict, self.prj_dict["find_replace_saveas_f_ex_skip"], 'warning')
    ...
    def ut_find_replace_saveas_invalid_replace_tpl(self, mock_log):
        ...
        self.assertFalse(result['check'])
        mock_log.assert_any_call(self.mpy_trace, self.prj_dict, self.prj_dict["find_replace_saveas_tpl_err"], 'error')